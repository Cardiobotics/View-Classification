...
def get_predict_args():
    ...
    parser = add_model_config_args(parser)
    parser = add_data_config_args(parser)
    parser = add_predict_config_args(parser)
...
def add_model_config_args(parser):
    ...
    group.add_argument('--model_name', type=str, default='inception', help='Used for model selection and logging',
                       choices=['inception', 'resnext', 'resnet'])
    group.add_argument('--n_outputs', type=int, default=5, help='The number of outputs from the model.')

    group.add_argument('--finetune', default=True, action='store_true', help='Finetuning enabled')
    group.add_argument('--pre_trained_checkpoint', type=str, help='checkpoint to load state dict from')
...
def add_predict_config_args(parser):
    group = parser.add_argument_group('predict', 'Predict settings')

    group.add_argument('--result_file', type=str, help='Filename and path to save results from predictions')
